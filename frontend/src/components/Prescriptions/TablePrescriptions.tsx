...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/prescriptions/prescriptionsSlice'
...
const TableSamplePrescriptions = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { prescriptions, loading, count, notify: prescriptionsNotify, refetch } = useAppSelector((state) => state.prescriptions)
  ...
  useEffect(() => {
      if (prescriptionsNotify.showNotification) {
          notify(prescriptionsNotify.typeNotification, prescriptionsNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `prescriptions`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
