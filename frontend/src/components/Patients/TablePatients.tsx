...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/patients/patientsSlice'
...
const TableSamplePatients = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { patients, loading, count, notify: patientsNotify, refetch } = useAppSelector((state) => state.patients)
  ...
  useEffect(() => {
      if (patientsNotify.showNotification) {
          notify(patientsNotify.typeNotification, patientsNotify.textNotification);
      }
  }, [patientsNotify.showNotification]);
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `patients`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
