...
import {setRefetch, uploadCsv} from '../../stores/appointments/appointmentsSlice';
...
const AppointmentsTablesPage = () => {
  ...
  const [filters] = useState([
          ...
          {label: 'Status', title: 'status', type: 'enum', options: ['Scheduled','Completed','Cancelled']},
  ]);
  const addFilter = () => {
        const newItem = {
            id: uniqueId(),
    ...
    const getAppointmentsCSV = async () => {
        const response = await axios({url: '/appointments?filetype=csv', method: 'GET',responseType: 'blob'});
    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox   className='mb-6' cardBoxClassName='flex flex-wrap'>
          <BaseButton className={'mr-3'} href={'/appointments/appointments-new'} color='info' label='New Item' />
          <BaseButton
              className={'mr-3'}
              color='info'
              label='Filter'
              onClick={addFilter}
          />
          <BaseButton className={'mr-3'} color='info' label='Download CSV' onClick={getAppointmentsCSV} />
            <BaseButton
              color='info'
              label='Upload CSV'
