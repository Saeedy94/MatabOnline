...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await PrescriptionsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await PrescriptionsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/', wrapAsync(async (req, res) => {
  ...
  const payload = await PrescriptionsDBApi.findAll(
    req.query, { currentUser }
  );
  if (filetype && filetype === 'csv') {
    const fields = ['id','tracking_code',
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await PrescriptionsDBApi.findBy(
    { id: req.params.id },
  );
